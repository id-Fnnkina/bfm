...
doGet(request,response);
    ...
    public void doPage(HttpServletRequest request, HttpServletResponse response)
        ...
        page.setTotalCount(pl.count("enterprisecount"));
        // 调用Service层进行分页查询
        pl.RecordsList(page,"enterprisecount");
        ...
        System.out.println("正在进行企业经营风险评估页面展示");
        // 将业务层处理后的分页对象存放至request作用域中
        request.setAttribute("page", page);
        request.getRequestDispatcher("/WEB-INF/pages/enterprise .jsp").forward(request, response);
    }
